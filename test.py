...
def gpt(text,user,types):
    ...
    for question_type in types:
        ...
        # 각 카테고리에 대해 데이터를 분류
        questions = re.findall(question_pattern, return_data)
        answers = re.findall(answer_pattern, return_data)
        explanations = re.findall(explanation_pattern, return_data)
...
3점슛 라인이 50cm 더 멀다. 현재 NBA의 3점슛 라인은 7.25m로 한 때는 FIBA의 규정보다 1m나 더 멀었으나, 2010년 여러 룰 개정을 하면서 FIBA가 3점슛 라인을 50cm 늘리면서 6.75m가 되어 현재는 50cm 차이가 난다. 한편 94-95 시즌부터 96-97 시즌, 이 세 시즌 동안 NBA에서 3점슛 라인을 50cm 줄인 6.75m로 유지한 적이 있다. 그래서 당시 뛰었던 선수들 중에 농구 황제 마이클 조던이나 3점 슛의 대가 레지 밀러 등은 짧아진 3점 슛 라인의 덕택을 많이 봤다. 여담으로 1984년도에 FIBA가 3점슛을 도입한 이후 쭉 6.25m 거리를 유지하다 2010년에 50cm 늘린 6.75m가 되어 현재에 이르고 있다.
FIBA 기준 3초 제한구역[3]]이 본래 사다리꼴이었으나, 2010년 위의 룰 개정 이후로 NBA처럼 직사각형으로 바뀌었다.
게임 시간이 쿼터당 12분이다. 즉, 한 게임에 도합 48분 경기를 한다. 이 때문에 타임아웃도 1개 더 많고, 파울 아웃도 5반칙이 아니라 6반칙이다. 또한 팀 파울 기준도 4개가 아닌 5개다.
경기 중 선수가 타임아웃을 부를 수 있다.[4]
